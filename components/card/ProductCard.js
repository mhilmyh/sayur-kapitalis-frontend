...
const ProductCard = (props) => {
  ...
  const isCategoryVisible = useMediaQuery(theme.breakpoints.up("sm"));
  console.log(`is category visible : ${isCategoryVisible}`);
  ...
  return (
    <React.Fragment>
      <div className="rounded overflow-auto shadow-lg sm:w-3/4 m-2 md:m-8 relative">
        <DiscountTag discount={props.promo_percentage}></DiscountTag>
        <ProductImage
          src={props.image_url}
          name={props.name}
        ...
        {!!isCategoryVisible === true && (
          <div className="py-2 absolute top-0 right-0">
            <span
              className="inline-block bg-gray-200 rounded px-2 py-1 text-sm font-semibold text-gray-700 mr-2 max-w-xs cursor-pointer"
              onClick={() => setOpen(true)}
            >
              {props.category.name}
            </span>
